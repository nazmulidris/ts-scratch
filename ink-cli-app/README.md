# README (don't forget to migrate assets folder to developerlife.com)
...
excerpt: |
  This handbook is a reference guide on how to use Ink v3.2.0 (with TypeScript and React) to create CLI apps.
  IDEA Ultimate / Webstorm project files are provided.
  This handbook is written as a reference.
  Simply jump the section that you're interested in from the TOC, there is no particular order to the sections.
layout: post
title: "Ink v3.2.0 (w/ React, Node.js and TypeScript) handbook"
categories:
  - TypeScript
  - React
  - Web
  ...
  - CLI
---
<img class="post-hero-image" src="{{ 'assets/ink-handbook.svg' | relative_url }}"/>
```

<img src="assets/ink-handbook.svg"/>
...
- [Install Ink and create a project scaffold in TypeScript](#install-ink-and-create-a-project-scaffold-in-typescript)
...
- [Make major changes to the scaffolding provided by `create-ink-app` v2.1.1](#make-major-changes-to-the-scaffolding-provided-by-create-ink-app-v211)
  - [Rename `source` to `src`](#rename-source-to-src)
  - [Update a few dependencies](#update-a-few-dependencies)
  - [Switch to ECMAScript modules for our node module and TypeScript strict mode](#switch-to-ecmascript-modules-for-our-node-module-and-typescript-strict-mode)
...
This handbook is a reference guide on how to use Ink v3.2.0 (_with TypeScript and React_) to create
CLI apps. IDEA Ultimate / Webstorm project files are provided. This handbook is written as a
reference. Simply jump the section that you're interested in from the TOC, there is no particular
order to the sections.

> ### ⚠️ This handbook isn't a reference for React, Node.js, TypeScript, or CSS
>
...
1. `host` components are platform specific components that belong to the host environment, eg:
   ...
   - For a browser / DOM host these could be things like `div` or `img` (aka regular DOM nodes).
     They typically begin with lower-case in the case of [`react-dom`][2.2].
...
[2.1]:
  https://github.com/r3bl-org/r3bl-ts-utils/blob/3251cdf13f029da641c9e467dad513f9a27abc47/src/color-console-utils.ts#L99
[2.2]: https://github.com/facebook/react/tree/main/packages/react-dom
[2.3]:
  https://developerlife.com/2021/07/02/nodejs-typescript-handbook/#user-input-and-output-via-stdin-stdout
...
## Install Ink and create a project scaffold in TypeScript
...
- This module is executable since the `bin` property value is `dist/cli.js`. You can run this module
  ...
  ```shell
  npm exec -c 'ink-cli-app --name=Foo'
  ```
...
1. **🧙 "Thing" #1 - List of globally installed modules updated**
   ...
   globally installed npm modules. Here are some notes on this.
...
<img src="assets/cli.tsx.uml.svg"/>

```yaml
TODO:
  - Convert img tag to Liquid syntax for developerlife.com
```
...
[4.1]:
  https://developerlife.com/2021/07/02/nodejs-typescript-handbook/#nodejs-installation-on-linux-using-brew
...
> [here](#install-ink-and-create-a-project-scaffold-in-typescript) called `ink-cli-app`. Before
> reading further:
>
> 1. Please read this [section](#install-ink-and-create-a-project-scaffold-in-typescript) to
>    understand what `package.json` file does and what `npm link` does.
...
We are going to use the sample app we are working on started
[here](#install-ink-and-create-a-project-scaffold-in-typescript) called `ink-cli-app`.

> ⚠ Please read this [section](#install-ink-and-create-a-project-scaffold-in-typescript) before
> moving on to understand what `package.json` file does and what `npm link` does as well.
...
## Make major changes to the scaffolding provided by `create-ink-app` v2.1.1

We are going to majorly deviate from code that was generated by the scaffolding.
...
### Switch to ECMAScript modules for our node module and TypeScript strict mode
...
> ⚠ Currently (as of Oct 2021) many libraries, like `Jest` or `ava` tend to use `require` at their
> boot up sequence assuming their config files are written in CJS (CommonJS modules) which then
> fails because we are using TS files which are compiled to EJM (which means `require` can't be
> used). Yikes! 😮
...
> ⚡ Here's a
> [commit](https://github.com/nazmulidris/ts-scratch/commit/5498b19985dc0031e55866b0168200fc13538f9f)
